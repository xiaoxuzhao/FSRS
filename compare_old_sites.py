...
file='site.csv'
afile='fsrs_sites(1000days_good_depth).csv'
###############
df=read_csv(input_dir+file)
dfh=read_csv(input_dir+afile)
Lat=df["Latitude"]
Lon=df["Longitude"]
index=df.icol(0)
lat=dfh["Latitude"]
...
x1,y1=my_map(dfh["Longitude"].values,dfh["Latitude"].values)
...
for i in range(len(x1)):
    plt.text(x1[i], y1[i],index1[i],fontsize=10,fontweight='bold', ha='center',va='center',color='white')
my_map.plot(x1, y1, 'bo', markersize=12)    
a.set_title('compare longterm sites',fontsize=20)
my_map.drawparallels(np.arange(40,80,0.5),labels=[1,0,0,1])
my_map.drawmeridians(np.arange(-180,180,0.5),labels=[1,1,0,1])
plt.savefig(save_dir+'sites_map_compare',dpi=200)
plt.show()
